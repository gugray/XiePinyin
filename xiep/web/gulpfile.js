...
// Copy index template to deploy folder
gulp.task('template', function() {
  return gulp.src('./index.tmpl')
      .pipe(gulp.dest('../deploy/'));
});

// Copy static assets to deploy folder
...
gulp.task('clean', function () {
  return del(['./client-build/*.css', './client-vue-build/*.*', '../deploy/static/*.*']);
});

// Default task: full clean+build.
gulp.task('default', gulp.series('svelte-pack', 'browserify', 'styles', 'template', 'copy',
    function (done) { done(); }));
